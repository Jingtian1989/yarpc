...
public class ProviderBean {
    ...
    public void checkConfig() {
        ...
        if (metadata.getTarget() == null) {
            throw new IllegalArgumentException("[RPC] target should not be null.");
        }
        //代理接口
        ...
        try {
            ...
            if (!proxyClass.isInterface()) {
                throw new IllegalArgumentException("[RPC] proxy should be interface.");
            }

            if (!proxyClass.isAssignableFrom(metadata.getTarget().getClass())) {
                throw new IllegalArgumentException("[RPC] target should implement proxy class.");
            }


        } catch (ClassNotFoundException e) {
            throw new IllegalArgumentException("[RPC] proxy class does not exist.");
        }
    }
